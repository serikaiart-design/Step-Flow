(()=>{
const ISSUE_URL='https://github.com/serikaiart-design/Step-Flow/issues/new';
...
box.innerHTML=`<summary>Обратная связь</summary><div class="sf-feedback-body"><p>Заметили ошибку, что-то непонятно или есть идея для улучшения? Напишите короткий отзыв — он откроется на странице проекта GitHub, где вы сможете проверить и отправить его.</p><form class="sf-feedback-form" novalidate><label>Тип отзыва<select name="type"><option>Ошибка на сайте</option><option>Предложение</option><option>Непонятный момент</option><option>Другое</option></select></label><label>Ваш отзыв<textarea name="message" maxlength="2000" placeholder="Опишите, что произошло или что можно улучшить" required></textarea></label><div class="sf-feedback-actions"><button type="submit">Продолжить в GitHub</button><span class="sf-feedback-hint">Перед отправкой текст можно проверить и дополнить скриншотом</span></div><div class="sf-feedback-status" role="status" aria-live="polite"></div></form></div>`;
