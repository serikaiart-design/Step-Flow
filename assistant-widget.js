...
function pushHistory(){if(session.caseId&&session.family&&session.state){session.history=session.history||[];session.history.push({family:session.family,state:session.state})}}
function start(family,state){if(session.caseId){pushHistory();session.family=family;session.state=state}else{session={caseId:'case_'+Date.now(),family,state,attempted:[],history:[]}};render()}
function move(state){if(session.state){pushHistory();session.attempted.push(session.family+':'+session.state)}session.state=state;render()}
function goBack(){if(!session.caseId||!session.history||!session.history.length)return;const prev=session.history.pop();session.family=prev.family;session.state=prev.state;msg('← Назад',true);render()}
function clear(){session={caseId:null,family:null,state:null,attempted:[],history:[]};updateBack()}
